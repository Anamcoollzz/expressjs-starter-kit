...
export async function charge(req: Request, res: Response) {
  ...
  if (payment_type === "bank_transfer") {
    ...
    params.gopay = { enable_callback: true, callback_url: req.body.callback_url || "https://example.com" };
...
export async function notification(req: Request, res: Response) {
  ...
  try {
    if (body.signature_key) {
      const raw = body.order_id + body.status_code + body.gross_amount + (process.env.MIDTRANS_SERVER_KEY as string);
